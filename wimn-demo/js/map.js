  ...
  PictureMarkerSymbol) 
	{
		...
		*****************************************************************/ 
		...
		let boundariesLayer = new MapImageLayer({
			url: "https://pca-gis02.pca.state.mn.us/arcgis/rest/services/maps/ISW/MapServer",
			title: "Boundaries",
			sublayers: [
				{
					id: 3,
...
*/
		...
		const searchFacilityLayer = new FeatureLayer({
			...
			listMode: "hide",
			popupTemplate: {
				title: "WIMN SItes. FKA - Industrial Stormwater Monitoring Location",
				content: "Site ID: {mpca_id_list} <br /> Site name: {name} <br />needs more work to replacate WIMN"
			},
			fields: [
				{
		...
		const searchResBuffer = new FeatureLayer({
			...
			fields: [
				...
				{
					name: "program_name_list",
					alias: "program_name_list",
					type: "string"
				}
			],
		...
		*****************************************************************/ 
		...
		url:
			"https://pca-gis02.pca.state.mn.us/arcgis/rest/services/WIMN/wimn_tempo/MapServer"
		});
      ...
      //  view.ui.add(bgExpand, "top-left");
		...
		*****************************************************************/ 
		let theURL = "https://pca-gis02.pca.state.mn.us/arcgis/rest/services/WIMN/wimn_tempo/MapServer/legend";
		esriRequest(theURL, {
			query: {
				f: 'json'
			},
			responseType: "json"
				...
				listItemCreatedFunction: function (event) {
					...
					if (item.title === 'Legend (WIMN/wimn_tempo)') {
						item.title = "Legend";
					} 
		...
		*****************************************************************/
		...
		const search = new Search({
			...
			sources: [
			{
				layer: new FeatureLayer({
					//url: "https://" + window.location.host + "/arcgis/rest/services/maps/ISW/MapServer/0",
					url: "https://pca-gis02.pca.state.mn.us/arcgis/rest/services/WIMN/wimn_tempo/MapServer/1",
					outFields: ["*"]
				}),
				searchFields: ["mpca_id_list"],
				displayField: "mpca_id_list", //"master_ai_name", // I can only put one field here.
				exactMatch: false,
				outFields: ["*"],
				name: "Site IDs",
				placeholder:  "5151",
		...
		function showResults(response) {
			...
			if (results.length === 0) {
				fContainer.innerHTML = "<p><br />No results found. Please check the permit number and try again.</p>";
        		showSearchResContainer();
				return;
			}
			...
			results.forEach(function (findResult, i) {
				...
				let facility = findResult.feature.attributes["name"];
				
				// the ISW code buffers a sites moni location not the site. This needs hacking - JE 4/2023

				if (findResult.layerId===0) { // facility layer
						// Get values of desired attributes
						let owner = findResult.feature.attributes["name"];
						let description = findResult.feature.attributes["activity_list"];
						...
						let graphic = new Graphic({
							...
							attributes: {'name': facility,'owner name':owner,'activity list':description}
						});
						facilities.push(graphic);
				} else { // monitoring locations layer
					// Get values of desired attributes
					let itemId = findResult.feature.attributes["site_id"];
					let moniDesc = findResult.feature.attributes["name"];
			...
			const addEdits = {
				addFeatures: facilities //moniGraphics
			};
			updateMoniLocFilter();
		...
		function buildResultsView(results,layer,moniDesc,itemId) {
			...
			if (results.features.length>0) {
				...
				contentAccordion.setAttribute('class','accordion__section');
				...
				resultDiv.setAttribute('aria-labelledby','table1Caption');
				if (showTable==true) {
					let resultTable = document.createElement('table');
					resultTable.setAttribute('class', 'tblResults')
					let caption = resultTable.createCaption();
					caption.textContent="Search Results";
					caption.id='table1Caption';
					//get the attributes for the table column
					let tableHead = buildTableHeader(desiredFieldsFriendlyNames);
					...
					for (let key in attributes) {
						...
						if(desiredFieldsFriendlyNames.includes("WID")==true && key==desiredFields[0]) {
							let nameWid = document.createElement('td')
							nameWid.innerText = attributes[desiredFields[0]] + ", WID " + attributes[desiredFields[1]];
							tableBodyRow.append(nameWid);
						} else if(desiredFieldsFriendlyNames.includes("WID")==true && key==desiredFields[1])  {
							//do not add it
						} else {
							let otherCell = document.createElement('td')
							otherCell.innerText = attributes[key];
							tableBodyRow.append(otherCell);
						}
				...
				requirementsDiv.setAttribute('class','reqts');
				requirementsDiv.innerHTML = customRptTxt2;
				
				contentAccordion.appendChild(resultDiv);
				contentAccordion.appendChild(requirementsDiv);
				contentAccordion.setAttribute('aria-hidden', true);
				contentAccordion.setAttribute('visibility', 'hidden');
				...
				btnAccordion.addEventListener('click', () => {
					...
					let table = associatedSection.getElementsByClassName("tblResults")[0];
					let permitLink = associatedSection.getElementsByClassName("permitLink")[0];
					if (btnAccordion.classList.contains('expanded')) {
						btnAccordion.setAttribute('aria-expanded', true);
						associatedSection.setAttribute('display', 'block');
						//associatedSection.setAttribute
						associatedSection.setAttribute('aria-hidden', false);
						//remove tabIndex -1 from permit links and tables
						if (table){
							table.setAttribute('tabIndex',0);
						}
						if (permitLink){
							permitLink.removeAttribute('tabIndex');
						}
						associatedSection.focus();
					} else {
						btnAccordion.setAttribute('aria-expanded', false);
						associatedSection.setAttribute('display', 'none');
						associatedSection.setAttribute('aria-hidden', true);
						//add tabIndex -1 to permit links and tables
						if (table){
							table.setAttribute('tabIndex',-1);
						}
						if (permitLink){
							permitLink.setAttribute('tabIndex',-1);
						}
					}
				})
		...
		function buildTableHeader(desiredFields) {
			...
			desiredFields.forEach(function(field) {
				...
				if (field !='WID') {
					...
					header.scope ="col"; // Creates the current header cell during a specific iteration
	...
	async function createPDF(reportArr,view) {
		...
		reportArr.forEach(function(moniRpt){
			...
			tables.forEach(function(layer){
				...
				let closingTxt = layer.closingTxt.replace("<a tabIndex=-1 class='permitLink' href='https://www.pca.state.mn.us/sites/default/files/wq-strm3-67i.pdf' target='_blank'> general permit</a>","general permit");
		...
		function pageFooter() {
			let date = curday();
			let footerTxt ="This report was generated on " + date + " using the Industrial Stormwater Special and Impaired Waters Search Tool: https://pca-gis02.pca.state.mn.us/ISW/";
